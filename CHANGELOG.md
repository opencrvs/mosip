...
- WebSub callbacks are now authenticated with the hub's `X-Hub-Signature` HMAC, and rejected with a 401 if it is missing or does not match. Previously nothing established that a callback came from MOSIP: the route is exempt from JWT auth, and successfully decrypting the credential only proves the sender had OpenCRVS's public certificate, which is not a secret. ([opencrvs-core#12842](https://github.com/opencrvs/opencrvs-core/issues/12842))
...
- Allow adding a custom `schemaJson` instead of hard-coded. ([e88e749](https://github.com/opencrvs/mosip/commit/e88e749))

- Added `jti` claim to the E-Signet request as required by E-Signet. ([35d4be8](https://github.com/opencrvs/mosip/commit/35d4be8))

- Aligned to E-Signet's new name handling in Collab. It now splits the name by ' ' instead of separate claims. ([8927bab](https://github.com/opencrvs/mosip/commit/8927bab))
